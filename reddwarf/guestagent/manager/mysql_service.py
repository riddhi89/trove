        ...
        with LocalSqlClient(get_engine()) as client:
            ...
            for db in db_result:
                ...
                if db['grantee'] == "'%s'@'%s'" % (user.name, user.host):
                    ...
                    databases_access.add(db_name)
        with LocalSqlClient(get_engine()) as client:
            uu = query.UpdateUser(user.name, host=user.host,
                                  clear=user_attrs['password'], 
                                  new_user=user_attrs['name'],
                                  new_host=user_attrs['host'])
            t = text(str(uu))
            LOG.debug('\n'*10)
            LOG.debug(t)
            LOG.debug('\n'*10)
            client.execute(t)
            if user_attrs['name'] is not None:
                if user_attrs['name'] not in grantee:
                    if user_attrs['host'] is None:
                        host = user.host
                    else:
                        host = user_attrs['host']
                    self.grant_access(user_attrs['name'], host, databases_access)    
